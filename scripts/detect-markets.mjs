...
async function isAvailableIn(pkg, code) {
  try {
    const timeout = new Promise((_, r) => setTimeout(() => r(new Error("timeout")), 8000))
    const result = await Promise.race([gplay.app({ appId: pkg, country: code.toLowerCase(), throttle: 10 }), timeout])
    return result.available === true
  } catch { return false }
}

async function detectForApp(app, index, total) {
  process.stdout.write(`[${index}/${total}] ${app.name} ... `)
  const available = []
  for (let i = 0; i < COUNTRIES.length; i += 15) {
    const batch = COUNTRIES.slice(i, i + 15)
    const results = await Promise.all(batch.map(async c => ({ code: c.code, ok: await isAvailableIn(app.packageName, c.code) })))
    for (const { code, ok } of results) if (ok) available.push(code)
  }
  if (available.length === 0) { console.log("no markets"); return }

  const existing = await prisma.market.findMany({ where: { code: { in: available } }, select: { id: true, code: true } })
  ...
  const created = await Promise.all(
    available.filter(c => !existingCodes.has(c)).map(code => {
      ...
      return prisma.market.create({ data: { code: info.code, name: info.name, region: info.region, flagEmoji: flagEmoji(info.code) }, select: { id: true, code: true } })
    })
  )
  const allMarkets = [...existing, ...created]
...
const apps = await prisma.app.findMany({
  where: { packageName: { not: null }, appMarkets: { none: {} } },
